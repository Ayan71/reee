...
import {
  TrendingUp, Zap, Code2, BrainCircuit, ShieldCheck, Headphones
} from 'lucide-react';
...
export default function WhyChooseUs() {
  ...
  return (
    <section id="why-us" className="section-padding bg-[#0a0a0a] relative overflow-hidden">
      ...
      <div className="container-custom relative z-10" ref={ref}>
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            >
              We don&apos;t just write code — we engineer competitive advantages. Every project is treated as a mission-critical product with standards that match the world&apos;s leading tech companies.
            </motion.p>
            ...
            >
              {[
                ...
                { label: 'Code coverage standard', value: '90%+' },
                { label: 'Client retention rate', value: '94%' },
              ].map((item) => (
                <div key={item.label} className="flex items-center justify-between glass-card rounded-xl px-6 py-4">
                  <span className="text-white/60 text-sm">{item.label}</span>
                  <span className="font-bold text-lg orange-text-gradient">{item.value}</span>
                </div>
              ))}
            </motion.div>
          </div>
            ...
            {whyChooseUs.map((item, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="group glass-card gradient-border rounded-2xl p-6 relative overflow-hidden cursor-default"
                >
                  {/* Hover gradient */}
